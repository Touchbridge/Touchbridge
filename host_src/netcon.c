...
int parse_uri(char *uri_in, char **addr, int *port)
{
    char *p;
    char *uri = uri_in;
    ...
    while (*uri && isspace(*uri)) uri++;

    if (strcasestr(uri, "tcp") != uri) {
    	fprintf(stderr, "%s: %s: only TCP supported\n", __FUNCTION__, uri_in);
    	return -1;
    }

    p = strstr(uri, "://");
    if (!p) {
    	fprintf(stderr, "%s: %s: expected URI of form: tcp://addr:port\n", __FUNCTION__, uri_in);
    	return -1;
    }
    uri = p+3;
...
int netcon_sock_connect_uri(char *uri)
{
    char *addr;
    int port;

    int ret = parse_uri(uri, &addr, &port);
    if (ret < 0) {
        return ret;
    }

    ret =  netcon_sock_connect(addr, port);
    free(addr);
    return ret;
}


int netcon_sock_close(int fd)
{
    return close(fd);
}
...
int netcon_sock_listen_uri(char *uri)
{
    char *addr;
    int port;
    ...
    ret =  netcon_sock_listen(addr, port);
    free(addr);
    return ret;
}
...
int netcon_main_loop(netcon_t *nc, int timeout)
{
    while (1) {
        int ret = netcon_poll(nc, timeout); // TODO: add periodic callback capability
        if (ret < 0) {
